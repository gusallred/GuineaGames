...
def generate_random_store_stock():
    ...
    # --- FIX: Define breeds mapped to coat lengths ---
    breeds = [
        ("American", "Short"),
        ("Abyssinian", "Long"),
        ("Peruvian", "Long"),
    ...
    for i in range(3): 
        ...
        fake_data = {
            ...
            "species": chosen_breed, # Use the Breed name as the species
            "coat_length": coat,
                    ...
                    if btn_rect.collidepoint(mouse_pos):
                        if user_balance >= listing['price']:
                            try:
                                ...
                                api.create_pet(
                                    # --- FIX: Use POSITIONAL arguments for the first 4 parameters ---
                                    # This matches how main.py calls it, preventing keyword mismatch bugs.
                                    user_id,                                # 1. Owner
                                    p_data['name'],                         # 2. Name
                                    p_data.get('species', 'American'),      # 3. Species/Breed
